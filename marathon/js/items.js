// Items Page JavaScript — Popout Modal System (V2)
// All items load on one page; clicking a card opens a detail popout.
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Type filter pills (delegated — pills built after API call)
    ...
    // Rarity filter pills
    document.getElementById('rarityFilterPills')?.addEventListener('click', (e) => {
        const pill = e.target.closest('.filter-pill-sm[data-filter]');
        if (!pill) return;
    ...
    // Cryo Archive toggle
    document.getElementById('cryoArchiveToggle')?.addEventListener('click', () => {
        document.getElementById('cryoArchiveToggle').classList.toggle('active');
        filterAndSortItems();
    });
...
async function loadItems() {
    try {
        // Prefer prebuilt data (injected by build/prebuild-items.js)
        if (window.__ITEMS_DATA && Array.isArray(window.__ITEMS_DATA) && window.__ITEMS_DATA.length > 0) {
            allItems = window.__ITEMS_DATA.slice();
...
function injectStructuredData(items) {
    ...
    const baseUrl = 'https://marathondb.gg/items/';

    // ItemList schema
    const itemList = {
        '@context': 'https://schema.org',
        '@type': 'ItemList',
        'name': 'Marathon Items Database',
        'description': 'Every item in Marathon — consumables, equipment, ammo, grenades, upgrades, and more.',
        'url': baseUrl,
    ...
    for (const item of items) {
        ...
        const answer = parts.length > 0
            ? parts.join(' ')
            : `${name} is an item found in Marathon.`;
...
function filterAndSortItems() {
    ...
    const activeRarity = document.querySelector('#rarityFilterPills .filter-pill-sm.active')?.dataset.filter || 'all';

    const cryoActive = document.getElementById('cryoArchiveToggle')?.classList.contains('active');

    filteredItems = allItems.filter(item => {
        if (cryoActive && !item.is_cryo_archive) return false;
        if (search) {
            ...
            <div class="core-card ${rarity}" data-slug="${escapeAttr(item.slug)}" role="button" tabindex="0">
                ...
                <div class="core-icon-container">
                    <div class="core-icon ${rarity}">
                        ${imgSrc
                            ? `<img src="${escapeAttr(imgSrc)}" alt="${escapeAttr(item.name)} – Marathon item" onerror="this.parentElement.classList.add('icon-fallback')" loading="lazy">`
...
function initItemShareButtons(item) {
    const pageUrl = window.location.href;
    const toastEl = document.getElementById('itemShareToast');
    const rarity = (item.rarity || 'standard').toLowerCase();
    ...
    const twitterBtn = document.getElementById('itemShareTwitter');
    const discordBtn = document.getElementById('itemShareDiscord');
    const copyBtn = document.getElementById('itemShareCopy');

    // Clone & replace to remove old listeners
    [twitterBtn, discordBtn, copyBtn].forEach(btn => {
        if (!btn) return;
    ...
    document.getElementById('itemShareTwitter')?.addEventListener('click', () => {
        const text = encodeURIComponent(itemName + ' — ' + rarityLabel + ' ' + itemType + ' on MarathonDB');
        window.open('https://x.com/intent/tweet?text=' + text + '&url=' + encodeURIComponent(pageUrl), '_blank', 'width=550,height=420');
    });

    document.getElementById('itemShareDiscord')?.addEventListener('click', async () => {
        try {
            await navigator.clipboard.writeText('**' + itemName + '** — ' + rarityLabel + ' ' + itemType + '\n' + pageUrl);
            showToast('Discord text copied!', 'toast-discord');
        } catch (e) { console.error('Copy failed', e); }
    });

    document.getElementById('itemShareCopy')?.addEventListener('click', async () => {
        try {
