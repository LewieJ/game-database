...
| Field | Type | Description |
|---|---|---|
| `give_item` | string | Name of the item to trade in |
| `give_quantity` | number | How many of `give_item` to trade |
...
- **Purchase Info section** — toggled on/off via a **Purchasable** toggle. When enabled, reveals:
  - **Vendor Listings** — one card per vendor, each containing:
    ...
    - **Barter Trades** — nested per-vendor; each row: Trade **N** × **[Item name]** for **N** of this item + optional rank requirement
      ...
      <div key={i} className="vendor-listing">
        ...
        {v.barter_trades.map((b, j) => (
          <div key={j} className="barter-trade">
            Trade {b.give_quantity}× {b.give_item} for {b.receive_quantity} of this item
