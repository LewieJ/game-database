...
│   └── 0004_add_purchasable.sql               # Adds is_purchasable, purchase_credits, armory_location, armory_faction_rank
│
├── scripts/
...
| `purchase_credits` | INTEGER | Credits cost to purchase. Nullable. |
| `armory_location` | TEXT | Armory faction store where item is sold. One of: `Cyac`, `Nucal`, `Traxus`, `Mida`, `Arachne`, `Sekgen`. Nullable. |
| `armory_faction_rank` | TEXT | Faction rank required to unlock the purchase (free-text, e.g. `Trusted`, `Rank 2`). Nullable. |
...
| `purchase_credits` | ❌ | Credits cost to purchase. Nullable. |
| `armory_location` | ❌ | One of: `Cyac`, `Nucal`, `Traxus`, `Mida`, `Arachne`, `Sekgen`. Nullable. |
| `armory_faction_rank` | ❌ | Faction rank required to unlock (free-text). Nullable. |
...
**Request body (JSON):** any subset of `name`, `item_type`, `rarity`, `description`, `image_url`, `acquisition_note`, `verified`, `value`, `known_locations`, `is_purchasable`, `purchase_credits`, `armory_location`, `armory_faction_rank`, `is_active`.
...
- **Purchase Info section** — toggled on/off via a **Purchasable** toggle. When enabled, reveals:
  - **Credits Required** — integer credit cost
  - **Armory Location** — dropdown: `Cyac`, `Nucal`, `Traxus`, `Mida`, `Arachne`, `Sekgen`
  - **Faction Rank Required** — free-text rank needed to unlock (e.g. `Trusted`, `Rank 2`)
...
| `is_purchasable` | `1` = item can be purchased. `0` = not purchasable. |
| `purchase_credits` | Credits required to buy the item. Nullable. Only relevant when `is_purchasable = 1`. |
| `armory_location` | Armory faction store that sells the item. One of: `Cyac`, `Nucal`, `Traxus`, `Mida`, `Arachne`, `Sekgen`. Nullable. |
| `armory_faction_rank` | Faction rank required to unlock the purchase (e.g. `Trusted`, `Rank 2`). Free-text. Nullable. |
...
export interface Item {
  ...
  armory_location:     string | null;  // 'Cyac' | 'Nucal' | 'Traxus' | 'Mida' | 'Arachne' | 'Sekgen' | null
  armory_faction_rank: string | null;
  is_active:           0 | 1;
...
Images are served with `Cache-Control: public, max-age=604800, immutable` — browsers and CDNs will cache them for 7 days automatically.
...
{item.is_purchasable === 1 && (
  <div className="purchase-info">
    {item.purchase_credits != null && <span>{item.purchase_credits} credits</span>}
    {item.armory_location   && <span>Armory: {item.armory_location}</span>}
    {item.armory_faction_rank && <span>Rank required: {item.armory_faction_rank}</span>}
  </div>
)}
```
