...
**Request body (JSON):** any subset of `name`, `item_type`, `rarity`, `description`, `image_url`, `acquisition_note`, `verified`, `value`, `known_locations`, `is_active`.
...
- **Create modal** — name auto-generates a slug preview, optional image upload in the same flow
- **Image upload flow:**
...
1. Create a new file in `migrations/` — name it `0004_description.sql`, `0005_...` etc.
2. Write the SQL (ALTER TABLE, CREATE TABLE, etc.)
3. Run `npx wrangler d1 migrations apply ITEMS_DB --remote`
