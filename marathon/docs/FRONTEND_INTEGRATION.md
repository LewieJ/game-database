...
If the OAuth callback fails, the backend returns a JSON error response instead of redirecting.
...
**Recommended approach:** If the backend redirects errors to `https://marathondb.gg/auth/create-account?error=token_exchange_failed`, the create-account page handles these gracefully with a user-friendly message rather than showing raw JSON.
